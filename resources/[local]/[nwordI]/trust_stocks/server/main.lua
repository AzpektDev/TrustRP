...
function getItemInStock(source, count, item_name, inventory_name)
	...
	if item_name == 'black_money' then
		...
		if newQty >= 0 then
			...
			xPlayer.showNotification("Wypłaciłeś ~g~" .. count .. "$ ~p~nieopodatkowanej gotówki")
		else
		...
		if newQty >= 0 then
			...
			xPlayer.showNotification("Wyciągnąłeś ~g~" .. count .. "  ~p~".. xPlayer.getInventoryItem(item_name).label)
			wiadomosc = "Wyciagnał z szafki \n[ITEM: "..item_name.." | ILOSC: x"..count.."] \n[ID: "..source.." | Nazwa: "..name2.." | SteamID: "..steamid.."]" 
			DiscordHookwyciaganie1('FoxRP.pl', wiadomosc, 11750815)
		else
...
function getMoney(source, owner, count, item_name, inventory_name)
	...
	if check[1] ~= nil then
		...
		if count <= pMoney then
			...
			xPlayer.addAccountMoney(item_name, count)
			TriggerClientEvent('esx:showNotification', _source, "Wyciągnąłeś ~g~$" .. count .. " ~p~nieopodatkowanej gotówki")
		end
	end
end
...
function putItemInStock(source, count, item_name, inventory_name)
	...
	if item_name == 'black_money' then
		...
		if check[1] ~= nil then
			...
			xPlayer.showNotification("Wpłaciłeś ~g~" .. count .. "$ ~p~nieopodatkowanej gotówki")
		else
		...
		if check[1] ~= nil then
			...
			xPlayer.showNotification("Włożyłeś ~g~" .. count .. " ~p~" .. xPlayer.getInventoryItem(item_name).label)
			local steamid = xPlayer.identifier
			local name2 = GetPlayerName(source)
			wiadomosc = "Włożył do szafki \n[ITEM: "..item_name.." | ILOSC: x"..count.."] \n[ID: "..source.." | Nazwa: "..name2.." | SteamID: "..steamid.."]" 
			DiscordHookwyciaganie3('FoxRP.pl', wiadomosc, 11750815)
		else
...
AddEventHandler('szymczakovv_stocks:putWeaponInStock', function(item, count, name)
	...
	DiscordHookwkladanie('FoxRP.pl', wiadomosc, 11750815)
end)
...
function DiscordHookwkladanie(hook,message,color)
    local wkladanie = 'https://discord.com/api/webhooks/878368503558717451/Y9Su1OrDotzIK3F9IUe4-EgC27tiIuIjjLuvHW3JyY5QHUOT4mLt3wsdLwUmgV5Qi3-E'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
function DiscordHookwkladanie2(hook,message,color)
    local wkladanie22 = 'https://discord.com/api/webhooks/878368503558717451/Y9Su1OrDotzIK3F9IUe4-EgC27tiIuIjjLuvHW3JyY5QHUOT4mLt3wsdLwUmgV5Qi3-E'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
function DiscordHookwyciaganie1(hook,message,color)
    local wyciaganie44 = 'https://discord.com/api/webhooks/878368503558717451/Y9Su1OrDotzIK3F9IUe4-EgC27tiIuIjjLuvHW3JyY5QHUOT4mLt3wsdLwUmgV5Qi3-E'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
