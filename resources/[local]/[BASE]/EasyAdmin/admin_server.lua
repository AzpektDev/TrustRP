...
AddEventHandler("SavellPlayer", function(source)
	...
	ESX.SavePlayers(cb)
		...
		print('[Wyspa]> Synchronizacja graczy przy użyciu narzędzia EasyAdmin gotowa.')
...
AddEventHandler("SavellPlayerAuto", function()
	...
	print('[Wyspa]> Auto Synchronizacja graczy gotowa.')
...
Citizen.CreateThread(function()
	...
	RegisterCommand("report", function(source, args, rawCommand)
		...
		DiscordHookReport('FoxRP', wiadomosc, 11750815)
		TriggerClientEvent('chatMessage', source, "^1[FoxRP]", {255,255,255}, GetLocalisedText("admincalled"))
	
	end, false)

	function DiscordHookReport(hook,message,color)
		local gigafajnywebhook = ''
		local embeds = {
					{
				["title"] = message,
				["type"] = "rich",
