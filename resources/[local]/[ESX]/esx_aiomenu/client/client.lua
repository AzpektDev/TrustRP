...
RegisterCommand('dowod', function(...)
  if GetGameTimer() > lastGameTimerId then
    TriggerServerEvent('dowod')
		...
		TriggerEvent("chatMessage", "^3[FoxRP]> ", {252, 244, 3}, "Nie możesz tak często wyciągać dowodu!")
	end
end)
...
RegisterNUICallback('psychologodznaka', function(...)
  if PlayerData.job.name == 'psycholog' then
    if GetGameTimer() > lastGameTimerpsychol then
      ...
      TriggerEvent("chatMessage", "^3[FoxRP]>  ", {252, 244, 3}, "Nie możesz tak często wyciągać legitymacji!")
      SetNuiFocus(false,false)
    end
  end
end)
...
RegisterCommand('wizytowka', function(...)
  if GetGameTimer() > lastGameTimerPhone then
	  TriggerServerEvent('wizytowka')
		...
		TriggerEvent("chatMessage", "^3[FoxRP]>  ", {252, 244, 3}, "Nie możesz tak często wyciągać wizytówki!")
  end
end)
...
AddEventHandler("menu:getSteamIdent", function(identity)
  if myIdentifiers.steamidentifier then
    TriggerEvent("chatMessage", "^1[IDENTITY]", {252, 244, 3}, "Your steam identifier is:" .. myIdentifiers.steamidentifier)
  else
    TriggerEvent("chatMessage", "^1[IDENTITY]", {252, 244, 3}, "Your steam identifier is nil. Please use /getID")
  end
end)
