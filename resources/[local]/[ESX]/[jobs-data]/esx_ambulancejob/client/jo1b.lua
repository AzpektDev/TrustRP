	...
	if CurrentAction ~= nil then
	  ...
	  if IsControlJustReleased(0, Keys['E']) then
		...
		elseif PlayerData.job ~= nil and (PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'offambulance') then
			...
			elseif CurrentAction == 'lsfd_boss' then
				local opt = {
					employees = false
				}
				if PlayerData.job.grade_name ~= 'boss' then
				...
				TriggerEvent('esx_society:openBossMenu', 'ambulance_extra', function(data, menu)
				  menu.close()
				end, opt)
			elseif PlayerData.job.name == 'ambulance' or (PlayerData.job.name == 'offambulance' and PlayerData.job.grade > 3) then
