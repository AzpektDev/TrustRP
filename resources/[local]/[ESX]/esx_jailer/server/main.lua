...
RegisterCommand('unjail', function(source, args, user)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer.group == 'admin' or xPlayer.group == 'superadmin' or xPlayer.group == 'moderator' or xPlayer.group == 'best' or xPlayer.group == 'support' or xPlayer.job.name == 'police' then
		if args[1] then
