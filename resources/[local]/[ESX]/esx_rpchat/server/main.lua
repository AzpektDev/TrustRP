...
AddEventHandler('sendTwtServer', function(message)
	...
	if mamhajsikbejbe33 > 100 then
		...
		xPlayer.showNotification('Nie posiadasz wystarczająco pieniędzy w banku')
	end
	...
	if args[1] ~= nil then
		...
		if xPlayer.job.name == 'police' then
			...
			if mahajs1 >= 0 then
				...
				TriggerClientEvent('esx:showNotification', source, '~g~Wysłano wiadomość')
			else
				TriggerClientEvent('esx:showNotification', source, '~r~Wysyłanie wiadomości się nie powiodło')
			end
		end
	end
end)
...
RegisterCommand('ems', function(source, args, rawCommand)
	...
	if args[1] ~= nil then
		...
		if xPlayer.job.name == 'ambulance' then
			...
			local mahajs1 = xPlayer.getMoney()
			if mahajs1 >= 0 then
				TriggerClientEvent('chatMessage', -1, "👨‍⚕️ @" ..name.firstname..' '..name.lastname.." ", {255, 46, 46}, msg)
				xPlayer.removeMoney(0)
				TriggerClientEvent('esx:showNotification', source, '~g~Wysłano wiadomość')
			else
				TriggerClientEvent('esx:showNotification', source, '~r~Wysyłanie wiadomości się nie powiodło')
...
RegisterCommand('lsc', function(source, args, rawCommand)
	...
	if args[1] ~= nil then
		...
		if xPlayer.job.name == 'mecano' then
			...
			local mahajs1 = xPlayer.getMoney()
			if mahajs1 >= 0 then
				TriggerClientEvent('chatMessage', -1, "👨‍🔧 @" ..name.firstname..' '..name.lastname.."", {255, 192, 46}, msg)
				xPlayer.removeMoney(0)
				TriggerClientEvent('esx:showNotification', source, '~g~Wysłano wiadomość')
			else
				TriggerClientEvent('esx:showNotification', source, '~r~Wysyłanie wiadomości się nie powiodło')
...
RegisterCommand('o', function(source, args, rawCommand)
	if args[1] ~= nil then
		...
		if xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'best' or xPlayer.group == 'moderator' then
			...
			msg = Emojit(table.concat(args, " "))
			TriggerClientEvent('chatMessage', -1, "🌐 ^1[FoxRP] ^1OGŁOSZENIE^7", {30, 144, 255}, msg)
...
RegisterCommand('opis', function(source, args, user)
	...
	if args[1] ~= nil then
		...
		TriggerClientEvent('esx:showNotification', source, '~r~Maksymalna długość opisu postaci wynosi ~h~60~h~ znaków!~w~')
			...
			opissunio('FoxRP.pl', wiadomosc, 11750815)
		end
	else
