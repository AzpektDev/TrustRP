...
function OpenOrganisationActionsMenu()
    ...
    'default', GetCurrentResourceName(), 'organisation_actions',
    {
		...
		if data.current.value == 'handcuffs' then
			menu.close()
			exports['trust_kajdanki']:HandcuffsAction()
...
CreateThread(function()
	while true do
		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			if IsControlJustReleased(0, 168) and GetEntityHealth(GetPlayerPed(-1)) > 100 and not exports['esx_policejob']:isHandcuffed() and PlayerData.hiddenjob and Config.Interactions[PlayerData.hiddenjob.name] then
